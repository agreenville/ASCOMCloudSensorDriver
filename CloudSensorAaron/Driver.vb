...
'
' ASCOM SafetyMonitor driver for CloudSensor
'
' Description:	ASCOM driver for Ardunio cloud sensor.
'				
'
' Implements:	ASCOM SafetyMonitor interface version: 1.0
' Author:		(AG) Aaron Greenville <your@email.here>
'
' Edit Log:
'
' Date			Who	Vers	Description
' -----------	---	-----	-------------------------------------------------------
' 27-Mar-2016	AG	1.0.0	Initial edit, from SafetyMonitor template
' ---------------------------------------------------------------------------------
'
'
' Your driver's ID is ASCOM.CloudSensor.SafetyMonitor
'
...
<Guid("71391af6-267b-4a96-886f-d7fd1c0ed3d5")>
<ClassInterface(ClassInterfaceType.None)>
Public Class SafetyMonitor

    ' The Guid attribute sets the CLSID for ASCOM.CloudSensor.SafetyMonitor
    ' The ClassInterface/None addribute prevents an empty interface called
    ...
    Friend Shared IsConnect As Boolean
...
#Region "Common properties and methods"
    ...
    Public ReadOnly Property Name As String Implements ISafetyMonitor.Name
        Get
            Dim s_name As String = "Cloud sensor"
...
#End Region
    ...
    Public s As String
    Private safeState As Boolean = False
    ...
    Public ReadOnly Property IsSafe() As Boolean Implements ISafetyMonitor.IsSafe
        Get
            ...
            Try
                ...
                If s = "SM_TRUE#" Then
                    TL.LogMessage("IsSafe", True.ToString())
                    safeState = True
                    Return True
                ElseIf s = "SM_FALSE#" Then
                    TL.LogMessage("IsSafe", False.ToString())
                    safeState = False
                    Return False
                Else
                    TL.LogMessage("IsSafe", "Unknown response from Safety monitor")
                    Return safeState
                End If
            Catch ex As COMException
                TL.LogMessage("IsSafe", "No response from Safety monitor")
            Catch ex As Exception
                TL.LogMessage("IsSafe", "some other problem")
            End Try
        End Get
    End Property
...
#End Region
    ...
    ''' </summary>
    Public ReadOnly Property IsConnected As Boolean '''private
        Get
            If Not objSerial Is Nothing Then
                If objSerial.Connected Then
                    ...
                    Try
                        s = objSerial.ReceiveTerminated("#").Replace(vbCrLf, "")
                        If s = "GOTMESS#" Then
                            TL.LogMessage("Connected", "ASCOM is connected to " + comPort.ToString)
                            connectedState = True
                        Else
                    ...
                    Catch ex As DriverException
                        TL.LogMessage("Connected", "Unknown response from Safety monitor " + s)
                    Catch ex As COMException
                        TL.LogMessage("Connected", "No response from Safety monitor")
                    Catch ex As Exception
                        TL.LogMessage("Connected", "Unknown problem")
                    End Try
                Else
                    connectedState = False
                End If
